...
namespace Shaftesbury.Functional.Utils
{
    ...
    public static class Functional
    {
        ...
        // unfold: (b -> (a, b) option) -> b -> a list
        public static List<A> Unfold<A, B>(Func<B, Option<Tuple<A, B>>> unspool, B seed)
        {
            #region Precondition
            if (unspool == null) throw new ArgumentNullException("unspool");
            #endregion
        ...
        */
        ...
        public static IEnumerable<T> Repeat<T>(this Func<int, T> f, int howMany)
        {
            ...
            if (f == null) throw new ArgumentNullException("f");
            #endregion
            return Enumerable.Range(1, howMany).Select(f);
        }
